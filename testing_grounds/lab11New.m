...
maxEncoderDuty = 15; % Set your duty cycle
nb.setMotor(3,maxEncoderDuty);
% 15% duty cycle seems to be the lowest that doesn't cause aliasing
...
% second is around 3.6k at a duty cycle of 15%, so the sample frequency must be at least that
% much. Could be twice that if I'm remembering the Nyquist Theorem correctly.
% The sign of the duty cycle corresponds to the direction in which the 
% motor spins.
...
while toc < runtime
    ...
    rpm =  (abs(vals.counts) / dt) * (60 / 1440);
    
    rpms(end+1) = rpm; % Collect for graphing
    
    
    error = rpm_targ - rpm; % Difference between our target and current rpm 
    ...
    control = (kp * error) + (ki * integral) + (kd * derivative); % Put the error terms and coefficients together
                                                                  % into one control signal!
    
    %Caps the motor duty cycle at +/- max encoder duty cycle
    ...
    nb.setMotor(3, control); % Send the control signal to the motor.
end
